...
import AnalyzeFileView from './AnalyzeFileView.tsx';
import Dashboard from './Dashboard.tsx';
import LiveConversation from './LiveConversation.tsx';
...
const App: React.FC = () => {
  ...
  const handleAnalysisComplete = (analysisPromise: Promise<Omit<AnalysisResult, 'id' | 'date'>>) => {
    const originalView = view; // Keep track of where we came from
    setView('processing');
    analysisPromise
      .then(analysisData => {
      ...
      .catch(error => {
        ...
        setView(originalView === 'processing' ? 'home' : originalView); // Go back on failure
      });
  };
